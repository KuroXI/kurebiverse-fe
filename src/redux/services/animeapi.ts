...
import { IResult, IAnime, IEpisode, ISource, IAiringSchedule } from "@/type/Anime";
...
export const animeApi = createApi({
	...
	endpoints: (builder) => ({
		...
		getAnimeUpcomingEpisodes: builder.query<IResult<IAnime>, QueryPagePerPage>({
			query: ({ page, perPage }: QueryPagePerPage) => `/upcoming?page=${page}&perPage=${perPage}`,
		}),
		getTrendingAnime: builder.query<IResult<IAnime>, QueryPagePerPage>({
			query: ({ page, perPage }: QueryPagePerPage) => `/trending?page=${page}&perPage=${perPage}`,
		}),
		getPopularAnime: builder.query<IResult<IAnime>, QueryPagePerPage>({
			query: ({ page, perPage }: QueryPagePerPage) => `/popular?page=${page}&perPage=${perPage}`,
		}),
