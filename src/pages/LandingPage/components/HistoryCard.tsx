...
export default function HistoryCard({ anime } : HistoryCardProps) {
  const { data } = useGetAnimeInfoQuery(anime.animeId);

  return data && (
    <a href={`/watch/${anime.animeId}?episodeId=${anime.episodeId}`}>
      <div
        ...
        <div className={"absolute flex flex-col gap-2 w-full px-2 bottom-4"}>
          ...
          <div className={"flex flex-col"}>
            <div className={"flex justify-between items-center"}>
              <h1 className={"text-secondary-foreground text-sm"}>
                {parseTime(anime.time.progress * 1000)}
              </h1>
              <h1 className={"text-secondary-foreground text-sm"}>
                {parseTime(anime.time.totalDuration * 1000)}
              </h1>
            </div>
            <div className={"relative"}>
              ...
              <div
                ...
                style={{ width: `${progressBar(anime.time.progress, anime.time.totalDuration)}%` }}
              />
            </div>
          </div>
        </div>
