...
type AnimeCardModalProps = {
  query: (data: string) => UseQueryHookResult<QueryDefinition<string, BaseQueryFn, never, AnimeInfo>>;
  id: string
}
    ...
    <>
      <Box className={"relative"}>
        <img
          src={data?.cover}
          alt={data?.id}
          className={"w-full min-h-[300px] max-h-[400px] bg-top bg-contain bg-no-repeat"}
          style={{
      ...
      <Box className={"grid md:grid-cols-3 grid-cols-2 px-12 py-8 gap-x-4"}>
        <Box className={"md:col-span-2"}>
          <Box className={"flex gap-4 pb-5 lg:text-lg md:text-md text-sm"}>
            ...
            <p className={"text-white pr-5 text-center font-bold"}>{data?.releaseDate}</p>
          </Box>
        ...
        <Box className={"lg:text-md md:text-sm text-sx"}>
          ...
          <p className={"text-neutral-600"}>
            Genres: <span className={"text-white"}>{data?.genres.map((genre) => genre).join(", ")}</span>
          </p>
        </Box>
      </Box>
    </>
