import { Box, CircularProgress, createTheme, ImageListItem, imageListItemClasses, ThemeProvider } from "@mui/material";
...
const LatestEpisodesPage = () => {
	...
	const fetchPosts = useCallback(async () => {
		const response = await fetch(`${import.meta.env.VITE_BASE_URL}/upcoming?page=${currentPage}&perPage=100`);
	...
	return (
		<ThemeProvider theme={theme}>
			<h3 className={"text-white font-bold xl:text-3xl lg:text-2xl mb-5 ml-5 mt-20"}>Latest Episodes</h3>
			<Box
				className={"m-5"}
				gap={1.5}
				sx={{
